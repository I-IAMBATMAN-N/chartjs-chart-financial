...
(function (global, factory) {
  ...
  class FinancialController extends chart_js.BarController {
    ...
    draw() {
      const me = this;
      console.log("me ctx", me._ctx);
      const chart = me.chart;
      const rects = me._cachedMeta.data;

      const fractalHighsArr = [];
      const fractalLowsArr = [];

      for (let index = 0; index < rects.length; index++) {
        if (index > 0 && index < rects.length - 1) {
          //
          if (
            rects[index].high > rects[index - 1].high &&
            rects[index].high > rects[index + 1].high
          ) {
            fractalHighsArr.push(rects[index]);
          }
          if (
            rects[index].low > rects[index - 1].low &&
            rects[index].low > rects[index + 1].low
          ) {
            fractalLowsArr.push(rects[index]);
          }
        }
      }

      console.log("fractalHighsArr", fractalHighsArr);

      console.log("fractalLowsArr", fractalLowsArr);
  ...
  console.log("chart_js.BarElement ", chart_js.BarElement);
  ...
  class FinancialElement extends chart_js.BarElement {
    ...
    getCenterPoint(useFinalPosition) {
      console.log("---------------");
      console.log("useFinalPosition", useFinalPosition);
      console.log("typeof getCenterPoint", typeof useFinalPosition);
    ...
    tooltipPosition(useFinalPosition) {
      console.log("---------------");
      console.log("useFinalPosition", useFinalPosition);
      console.log("typeof tooltipPosition", typeof useFinalPosition);
    ...
    draw(ctx) {
      ...
      if (close < open) {
        ...
        borderColor = helpers.valueOrDefault(
          ...
          chart_js.defaults.elements.candlestick.borderColors.unchanged
        );
        ctx.fillStyle = helpers.valueOrDefault(
          me.backgroundColors ? me.backgroundColors.unchanged : undefined,
          chart_js.defaults.elements.candlestick.backgroundColors.unchanged
        );
      }
    ...
    draw(ctx) {
      ...
      if (close < open) {
        ctx.strokeStyle = helpers.valueOrDefault(
          me.options.borderColors ? me.options.borderColors.up : undefined,
          defaults.elements.ohlc.borderColors.up
        );
      } else if (close > open) {
        ctx.strokeStyle = helpers.valueOrDefault(
          me.options.borderColors ? me.options.borderColors.down : undefined,
